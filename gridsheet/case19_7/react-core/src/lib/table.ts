...
export class Table implements UserTable {
  ...
  public render(props: RendererCallProps) {
    ...
    const cell = this.getCellByPoint(point, "SYSTEM") ?? {};
    /*
    if (cell.renderer  == "checkbox") {
         console.log("cell.renderer:",point.x, point.y , cell.renderer);
          console.log(this.renderers[cell.renderer]);
    const renderer = this.renderers[cell.renderer ?? ""] ?? defaultRenderer;
    let r =  renderer.call({ table: this, point, sync });
    console.log("r", r)
    return r;
