...
import { a2p, r2y, c2x } from '../lib/converters';
...
export class Table implements UserTable {
  ...
  public spanList: SpanElementType[] = [];
  ...
  public initialize(cells: CellsByAddressType) {
    ...
    Object.keys(cells).forEach((address) => {
	  ...
	  const p = a2p(address);
	  this.spanList.push({ x:p.x, y:p.y, col_size:data.colsize??1, row_size:data.rowsize??1}); 
      }
      else if ( "colsize" in data ) {
          console.log("C:  ",address, a2p(address).x, a2p(address).y, data);
	  const p = a2p(address);
	  this.spanList.push({ x:p.x, y:p.y, col_size:data.colsize??1, row_size:1           }); 
      }
      else if ( "rowsize" in data ) {
          console.log("R:  ",address, a2p(address).x, a2p(address).y, data);
	  const p = a2p(address);
	  this.spanList.push({ x:p.x, y:p.y, col_size:1           , row_size:data.rowsize??1}); 
      }
